<script lang="ts">
	...
	let inUseCount = $derived(store.data.lockers.filter((l) => l.status === 'in-use').length);
...
<dialog bind:this={dialog} onclose={handleDialogClose}>
	{#if selectedLocker}
		...
		<div class="modal-body">
			...
			{:else}
				...
				<div class="info-row">
					...
					<span class="value">{formatDate(selectedLocker.assignedAt ?? '')}</span>
				</div>
			{/if}
		</div>
		...
		<div class="modal-footer">
			...
			{#if selectedLocker.status === 'available'}
				<button class="btn primary" disabled={!qrValid} onclick={handleGiveKey}>Give Key</button>
			{:else}
				<button class="btn danger" onclick={handleFreeLocker}>Free Locker</button>
			{/if}
		</div>
	{/if}
</dialog>
