<script lang="ts">
  ...
  let availableCount = $derived(
    store.data.lockers.filter((l) => l.status === "available").length,
  );
  let inUseCount = $derived(
    store.data.lockers.filter((l) => l.status === "in-use").length,
  );

  let partitionGroups = $derived(
    store.data.settings.partitions
      .map((p) => ({
        partition: p,
        lockers: store.data.lockers.filter((l) => l.partitionId === p.id),
      }))
      .filter((g) => g.lockers.length > 0),
  );

  let qrConflict = $derived.by(() => {
    const code = qrCode.trim();
    if (!code) return null;
    const conflict = store.data.lockers.find(
      (l) => l.status === "in-use" && l.assignedQR === code,
    );
    return conflict ? store.lockerLabel(conflict) : null;
  });

  let qrValid = $derived.by(() => {
    const code = qrCode.trim();
    if (!code) return false;
    if (qrConflict) return false;
    try {
  ...
  function openLocker(locker: Locker) {
    selectedLocker =
      store.data.lockers.find(
        (l) => l.partitionId === locker.partitionId && l.index === locker.index,
      ) ?? null;
    qrCode = "";
    dialog?.showModal();
  ...
  async function handleGiveKey() {
    if (!selectedLocker || !qrValid) return;
    await store.assignLocker(selectedLocker, qrCode.trim());
    closeModal();
  }

  async function handleFreeLocker() {
    if (!selectedLocker) return;
    await store.freeLocker(selectedLocker);
    closeModal();
  }

  function allocateNext() {
    const next = store.data.lockers.find((l) => l.status === "available");
    if (next) openLocker(next);
  }
...
<div class="page">
  ...
  {:else}
    <div class="partitions">
      {#each partitionGroups as { partition, lockers }}
        <div class="partition-section">
          <h2 class="partition-label">{partition.name}</h2>
          <div class="grid">
            {#each lockers as locker (locker.index)}
              <button
...
<dialog bind:this={dialog} onclose={handleDialogClose}>
  {#if selectedLocker}
    <div class="modal-header">
      <h2>{store.lockerLabel(selectedLocker)}</h2>
    ...
    <div class="modal-body">
      {#if selectedLocker.status === "available"}
        ...
        />
        {#if qrConflict}
          <span class="qr-error">Already assigned to locker {qrConflict}.</span>
        {:else if qrCode.trim().length > 0 && !qrValid}
    ...
    <div class="modal-footer">
      ...
      {#if selectedLocker.status === "available"}
        <button class="btn primary" disabled={!qrValid} onclick={handleGiveKey}
          >Give Key</button
        >
      {:else}
